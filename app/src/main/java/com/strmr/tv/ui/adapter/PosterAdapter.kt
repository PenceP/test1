...
class PosterAdapter(
    ...
    inner class PosterViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(item: ContentItem, position: Int) {
            ...
            // or they're standard Trakt media lists (Movie Collection, etc.)
            val hasTraktUrl = item.imdbId?.startsWith("https://trakt.tv/") == true
            val isTraktMediaList = com.strmr.tv.data.model.trakt.TraktMediaList.fromId(item.imdbId) != null
            val isPlaceholder = item.isPlaceholder || (item.tmdbId == -1 && !hasTraktUrl && !isTraktMediaList) || artworkUrl.isNullOrBlank()
            ...
            val progress = item.watchProgress
                ?: com.strmr.tv.data.repository.WatchStatusProvider.getProgress(item.tmdbId, item.type)
